...
interface Q1FollowUpPageTwoProps {
  npsScore: NpsScore;
  value: string;
...
export const Q1FollowUpPageTwo: React.FC<Q1FollowUpPageTwoProps> = ({
  npsScore,
  value,
  onChange,
  onNext,
  ...
  const questionTitle = isDetractorOrPassive
    ? 'Q1a. How can the Relationship Manager improve his/her services to make you give a rating of 10?'
    : 'Q1b. What did you specifically like about the services provided by Bank’s Relationship Manager?';

  const placeholderText = isDetractorOrPassive
    ? 'e.g. Quicker response time on phone calls, clearer guidance on investment products, regular portfolio updates...'
    : 'e.g. Highly knowledgeable, prompt query resolution, polite and proactive communication...';
        ...
        <div className="bg-white rounded-2xl p-3.5 sm:p-4 shadow-card border border-bankBorder">
          <div className="flex items-center gap-1.5 mb-2">
            <span className="text-[11px] font-extrabold text-emerald-800 uppercase tracking-wider bg-emerald-50 px-2 py-0.5 rounded-md border border-emerald-200">
              {isDetractorOrPassive ? 'Feedback Improvement' : 'Positive Highlights'}
            </span>
          </div>
